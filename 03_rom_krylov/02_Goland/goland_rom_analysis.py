...
case_rom_info = 'rom_MIMORA_r%d_sig%04d_%04dj' % (r, frequency_continuous_k[-1].real*100, frequency_continuous_k[-1].imag*100)
fig_folder = './figures/'
...
gain_struct_2_aero = np.block([[aeroelastic_system.Kdisp[:, :-10], aeroelastic_system.Kdisp_vel[:, :-10]],
...
save_dir = './res/' + case_nlin_info + case_rom_info
try:
    os.mkdir(save_dir)
...
np.save(save_dir + '/u_inf', uinf_part_plot)
np.save(save_dir + '/e_real', real_part_plot)
np.save(save_dir + '/e_imag', imag_part_plot)
