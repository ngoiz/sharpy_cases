...
import sharpy.rom.krylovreducedordermodel as krom
import sharpy.rom.frequencyresponseplot as fplot
...
rom = krom.KrylovReducedOrderModel()
...
frequency_response_plot = fplot.FrequencyResponseComparison()

plot_settings = {'frequency_type': 'k',
                 'plot_type': 'real_and_imaginary_siso'}
...
# frequency_response_plot.plot_frequency_response(kv, Yfreq_dummy_all, None, frequency_continuous_k)
frequency_response_plot.savefig('./figs/theo_rolled/UVLM_ROM_M%d_r%d.pdf' % (M, r))
# frequency_response_plot.savefigure('./figs/theo_rolled/DRA_01_06_r3.pdf')
# Y_freq_rom = rom.ssrom.freqresp(wv)

# Plotting

fig, ax = plt.subplots()
ax.semilogy(kv, np.abs(Yfreq_dummy_all[0,0,:].real - Y_freq_rom[0,0,:].real), lw=2, color='k', label='Real')
ax.semilogy(kv, np.abs(Yfreq_dummy_all[0,0,:].imag - Y_freq_rom[0,0,:].imag), lw=2, color='k', ls='-.', label='Real')

ax.set_xlabel('Reduced Frequency, k')
ax.set_ylabel('Absolute Error')

fig.savefig('./figs/theo_rolled/Error_1_M%d_r%d' % (M,r))


frequency_continuous_k = np.array([0.0, 0.2j])
# frequency_continuous_k = np.array([0.3j])
frequency_continuous_w = 2 * Uinf0 * frequency_continuous_k / ws.c_ref
# frequency_rom = np.array([1.9, 2.5])
# frequency_rom = 1.05
...
# cProfile.run('rom.run("algorithm|r|frequency_rom")')
rom.run(algorithm, r, frequency_rom)
Y_freq_rom2 = rom.ssrom.freqresp(wv)

fig, ax = plt.subplots()
ax.semilogy(kv, np.abs(Yfreq_dummy_all[0,0,:].real - Y_freq_rom[0,0,:].real), lw=2, color='k', label=r'Real - $\sigma=0$ - r=6')
ax.semilogy(kv, np.abs(Yfreq_dummy_all[0,0,:].imag - Y_freq_rom[0,0,:].imag), lw=2, color='k', ls='-.', label='Imag')

ax.semilogy(kv, np.abs(Yfreq_dummy_all[0,0,:].real - Y_freq_rom2[0,0,:].real), lw=2, color='b', label=r'Real - $\sigma=0,0.2j$ - r=3')
ax.semilogy(kv, np.abs(Yfreq_dummy_all[0,0,:].imag - Y_freq_rom2[0,0,:].imag), lw=2, color='b', ls='-.', label='Imag')

ax.set_xlabel('Reduced Frequency, k')
ax.set_ylabel('Absolute Error')
ax.legend()

fig.savefig('./figs/theo_rolled/ErrorComparison_1_M%d_r%d.pdf' % (M,r))
