...
left_vector = right_vector  #np.block([[1, 0], [0, 1]])
# left_vector.shape = (2, 1)
rom.restart_arnoldi = False
...
for i in range(2):
    for j in range(2):
        ...
        ax[i, j].semilogx(kv, Yfreq_dummy_all[i, j, :].real - Y_freq_rom[i, j, :].real, ls='-',
		color='k',
		label='Real')
        ax[i, j].semilogx(kv, Yfreq_dummy_all[i, j, :].imag - Y_freq_rom[i, j, :].imag,
		ls='-.',
		color='k',
		label='Imag')
        ...
        # ax[i, j].set_xlim([0,1])
ax[0,0].legend()
ax[0, 0].set_ylabel('To Output [1]')
ax[1, 0].set_ylabel('To Output [2]')
ax[0, 0].set_title('From Input [1]')
ax[0, 1].set_title('From Input [2]')
ax[1, 0].set_xlabel('Reduced Frequency, k')
