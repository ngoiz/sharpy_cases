...
plt.matshow(beam.U.T.dot(beam.Mstr.dot(beam.U)))
plt.show()
# # # Update modes
...
# Damped modes
Minv = np.linalg.inv(beam.Mstr)
nMinvK = -Minv.dot(beam.Kstr)
nMinvC = -Minv.dot(beam.Cstr)
nx = beam.Mstr.shape[0]
A = np.block([[np.zeros((nx, nx)), np.eye(nx)],
              [nMinvK, nMinvC]])

deigs, devecs_l, devecs_r = sclalg.eig(A, left=True, right=True)
order = np.argsort(deigs.real)[::-1]
deigs = deigs[order]
# devecs = devecs[:, order]
...
#     plt.plot(beam.U[ind_w1, i])
#     plt.plot(beam.U[ind_w2, i])
#     plt.scatter(np.arange(10), beam.U[-10:, i])
#     plt.show()
