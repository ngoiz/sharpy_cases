...
rbm_indices = np.arange(0, 6)
num_rbm = len(rbm_indices)
...
zero_matrix = np.zeros((3*uvlm.Kzeta, beam.num_dof - (10 - num_rbm)))
KdispS = aeroelastic_system.Kdisp[:, :-10]
KdispR = aeroelastic_system.Kdisp[:, -10:][:, rbm_indices]
Kdisp = np.hstack((KdispS, KdispR))

KvelS = aeroelastic_system.Kvel_disp[:, :-10]
KvelR = aeroelastic_system.Kvel_disp[:, -10:][:, rbm_indices]
Kveldisp = np.hstack((KvelS, KvelR))

KvelvelS = aeroelastic_system.Kvel_vel[:, :-10]
KvelvelR = aeroelastic_system.Kvel_vel[:, -10:][:, rbm_indices]
Kvelvel = np.hstack((KvelvelS, KvelvelR))

# Kas = np.block([[Kdisp, zero_matrix],
#                      [aeroelastic_system.Kvel_disp, aeroelastic_system.Kvel_vel]])
Kas = np.block([[Kdisp, zero_matrix],
                [Kveldisp, Kvelvel]])


Ksa = np.vstack((aeroelastic_system.Kforces[:-10, :], aeroelastic_system.Kforces[-10:, :][rbm_indices]))
...
Mrr = beam.Mstr[-10:, -10:]
Mrs = beam.Mstr[-10:, :-10]
beam.Mstr = np.block([[Mss, Msr[:, rbm_indices]],
                     [Mrs[rbm_indices, :], Mrr[rbm_indices, :][:, rbm_indices]]])
# beam.Mstr = beam.Mstr[:-4, :-4]

Kss = beam.Kstr[:-10, :-10]
Ksr = beam.Kstr[:-10, -10:]
Krr = beam.Kstr[-10:, -10:]
Krs = beam.Kstr[-10:, :-10]
beam.Kstr = np.block([[Kss, Ksr[:, rbm_indices]],
                      [Krs[rbm_indices, :], Krr[rbm_indices, :][:, rbm_indices]]])

Css = beam.Cstr[:-10, :-10]
Csr = beam.Cstr[:-10, -10:]
Crr = beam.Cstr[-10:, -10:]
Crs = beam.Cstr[-10:, :-10]
beam.Cstr = np.block([[Css, Csr[:, rbm_indices]],
                      [Crs[rbm_indices, :], Crr[rbm_indices, :][:, rbm_indices]]])
# Recalculate modes and frequencies
eigs, eigenvectors = np.linalg.eig(np.linalg.solve(beam.Mstr, beam.Kstr))
omega = np.sqrt(eigs)
order = np.argsort(omega)[:num_modes]
beam.freq_natural = omega[order]
dfact = np.diag(np.dot(eigenvectors.T, np.dot(beam.Mstr, eigenvectors)))
eigenvectors = (1./np.sqrt(dfact))*eigenvectors
beam.U = eigenvectors[:, order]


# orig_M = beam.U.T.dot(beam.Mstr.dot(beam.U))
# orig_K = beam.U.T.dot(beam.Kstr.dot(beam.U))

# Reorder modes
phiRR = np.eye(num_rbm)
phiSR = beam.U[:-num_rbm, :num_rbm]
phiRS = beam.U[-num_rbm:, num_rbm:]
phiSS = beam.U[:-num_rbm, num_rbm:]

beam.freq_natural = np.concatenate((beam.freq_natural[num_rbm:], beam.freq_natural[:num_rbm]))
phi = np.block([[phiSS, phiSR], [phiRS, phiRR]])

# Remove the quaternion
phi = phi
beam.U = phi

# new_M = beam.U.T.dot(beam.Mstr.dot(beam.U))
# new_K = beam.U.T.dot(beam.Kstr.dot(beam.U))
...
uvlm.Nu -= 3*uvlm.Kzeta
uvlm.SS.inputs = uvlm.Nu
#
...
# # Update modes - in order to include the stiffening terms in the solution
eigs, eigvecs = sclalg.eig(sclalg.solve(beam.Mstr, beam.Kstr))
omega = np.sqrt(eigs)
order = np.argsort(eigs)[:beam.Nmodes]
beam.freq_natural = omega[order]
...
ss_aeroelastic = libss.couple(ss01=uvlm.SS, ss02=beam_ss, K12=Kas, K21=Ksa)
