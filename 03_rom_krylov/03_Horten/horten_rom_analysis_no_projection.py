...
# zero_matrix = np.zeros((3*uvlm.Kzeta, beam.num_dof-4))
# Kas= np.block([[aeroelastic_system.Kdisp[:, :-4], zero_matrix],
#                [aeroelastic_system.Kvel_disp[:, :-4], aeroelastic_system.Kvel_vel[:, :-4]]])
#
# Ksa = aeroelastic_system.Kforces[:-4, :]
...
# beam.Cstr = beam.Cstr[:-4, :-4]
# beam.Kstr = beam.Kstr[:-4, :-4]

# orig_M = beam.U.T.dot(beam.Mstr.dot(beam.U))
# orig_K = beam.U.T.dot(beam.Kstr.dot(beam.U))

# Reorder modes (the eigenvector matrix comes ordered by frequency, hence the RBMs are the first since wn = 0)
phiRR = np.eye(10)
phiSR = beam.U[:-10, :10]
phiRS = beam.U[-10:, 10:]
phiSS = beam.U[:-10, 10:]

# Remove quaternion modes
# phiRR = np.eye(4)
# phiSR = beam.U[:-4, :4]
# phiRS = beam.U[-4:, 4:]
# phiSS = beam.U[:-4, 4:]

# beam.freq_natural = np.concatenate((beam.freq_natural[10:], beam.freq_natural[:10]))
beam.freq_natural = np.concatenate((beam.freq_natural[4:], beam.freq_natural[:4]))
