...
# # trim sigma = 1.5
# alpha = 4.200711359303956*np.pi/180
# beta = 0
# roll = 0
# gravity = 'on'
# cs_deflection =  -1.194130447084976*np.pi/180
# rudder_static_deflection = 0.0
# rudder_step = 0.0*np.pi/180
# thrust = 4.9130963575251485
# sigma = 1.5
# lambda_dihedral = 20*np.pi/180

initial_position = np.array([-30, 200, 10])

# trim sigma = 100
alpha = 3.871319970566575*np.pi/180
beta = 0
roll = 0
gravity = 'on'
cs_deflection = -0.7002191065285696*np.pi/180
rudder_static_deflection = 0.0
rudder_step = 0.0*np.pi/180
thrust = 4.618717252693335
...
def generate_solver_file():
    ...
    settings['PlotFlowField'] = {'postproc_grid_generator': 'MovingGridBox',
                                 'postproc_grid_input': {'x0': -20,
                                                         'y0': -10,
                                                         'z0': -10,
                                                         'x1': 15,
                                                         'y1': 10,
                                                         ...
                                                         'dy': 10.0,
                                                         'dz': 2.0},
                                 'velocity_field_generator': settings['StepUvlm']['velocity_field_generator'],
                                 'velocity_field_input': settings['StepUvlm']['velocity_field_input'],
                                 'dt': dt,
                                 'include_external': 'on',
                                 'include_induced': 'on'
                                 }
    settings['DynamicCoupled'] = {'structural_solver': 'NonLinearDynamicCoupledStep',
                                  ...
                                  'postprocessors_settings': {'PlotFlowField': settings['PlotFlowField'],
							      'BeamLoads': {'folder': route + '/output/',
                                                                            'csv_output': 'off'},
                                                              ...
                                                              'AerogridPlot': {
                                                                  ...
                                                                  'include_applied_forces': 'on',
                                                                  'minus_m_star': 0},
                                                              'CreateSnapshot': {'frequency': 100,
                                                                                 'keep': 5},
                                                              'Cleanup': {'remaining_steps': 2000},
                                                              'WriteVariablesTime': {
