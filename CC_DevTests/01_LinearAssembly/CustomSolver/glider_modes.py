...
ws.clean_test_files()
ws.update_mass_stiffness(sigma=1, lumped_mass_node=0, lumped_mass_=0,
...
ws.config['LinDynamicSim'] = {'dt': ws.dt,
                              ...
                              'postprocessors_settings': {'AerogridPlot': {
                                                              ...
                                                              'minus_m_star': 0},
                              'BeamPlot': {'folder': ws.case_route + '/output/',
                                'include_rbm': 'on',
                                'include_applied_forces': 'on'}}}

input_vec = np.zeros((ntsteps, 606))
...
def generate_linear_files(ws, input_vec):
    with h5.File(ws.case_route + '/' + ws.case_name + '.lininput.h5', 'a') as h5file:
        x0 = h5file.create_dataset(
            # 'x0', data=np.zeros((138)))
            'x0', data=np.zeros((1860)))
